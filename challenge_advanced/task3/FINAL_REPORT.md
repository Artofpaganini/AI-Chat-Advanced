...
Главный итог прогона: **приложение оказалось крепче, чем инструменты, которыми его проверяли**. Все реальные
блокеры пришли со стороны драйверов, а не кода.

| Инструмент | Таргет | Итог |
|---|---|---|
| `claude-in-mobile` | android | **рабочий**, весь L2 на android прогнан им |
| **Playwright** | web | **рабочий**, завёлся с первой попытки |
| `claude-in-mobile`, модуль `browser` | web | **не годится** для Compose/wasm |
| `claude-in-chrome` | web | **не подключается**, ноль полезных прогонов |

**`claude-in-chrome` не завёлся ни разу.** За три с лишним рестарта сессии `tabs_context_mcp` неизменно
отвечал «Browser extension is not connected». Провал связной, не функциональный: MCP-сервер поднимался,
расширение в браузере до него не достучалось. Из документации проекта убран как web-драйвер.

**Модуль `browser` у claude-in-mobile провалился функционально.** В `src/browser/client.ts` жёстко зашит
`--disable-gpu`, переменной для своих флагов нет. Дальше цепочка: Chrome отдаёт софтверный рендеринг ->
Skiko просит контекст с `failIfMajorPerformanceCaveat: true` -> получает `null` **молча, без исключения**
-> Compose не создаёт canvas. Пустая страница и чистая консоль - худший для диагностики вид поломки.
Обёртка через `CHROME_PATH`, выкидывающая флаг, вернула настоящий GPU (`ANGLE Metal Renderer: Apple M4 Pro`,
контекст с caveat-проверкой стал `OK`), но canvas всё равно не появился. Значит поверх GPU есть ещё причина,
и дальше копать в чужой инструмент нецелесообразно - дешевле взять Playwright.

**Playwright закрыл web полностью.** Отрисовал приложение сразу, принял ввод кириллицей без плясок с буфером
обмена, пишет скриншоты прямо в файл - у claude-in-mobile артефакты приходилось снимать через `adb`.

**Установка claude-in-mobile - отдельная ловушка.** Регистрировать строго
`npx -y claude-in-mobile@latest`. Без `@latest` npx подхватывает из PATH одноимённый brew-бинарь (это Rust
CLI, а не MCP-сервер), и сервер отдаёт **ноль тулов**. Со стороны выглядит как «подключился, но пустой».

**Ошибка измерения на стороне тестировщика.** Значительная часть времени ушла не на инструмент, а на неверный
индикатор: проверялся `document.querySelector('canvas')`, тогда как Compose кладёт canvas **в shadow DOM**,
где селектор его не находит никогда. Вывод «после починки GPU всё ещё сломано» был сделан по неверному
измерению. Проверять надо обходом shadow-root либо скриншотом.

**Рекомендуемая связка на будущее:** мобильные таргеты - `claude-in-mobile`, web - `Playwright`.
Это и зафиксировано в профиле `smoke`.
