...
После рестарта сессии `browser:open` должен поднять настоящий Chrome → WebGL появится → canvas отрисуется →
5 сценариев прогоняются координатным вводом. Альтернатива — расширение `claude-in-chrome` (тоже требует
подключения на стороне пользователя).
