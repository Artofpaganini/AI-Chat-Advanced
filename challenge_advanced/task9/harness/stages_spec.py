...
F_CONTEXT_AGE = "CONTEXT_AGE_MONTHS"

STAGE1_FIELDS = (F_AGE, F_SYMPTOMS, F_METRICS, F_DURATION, F_PARENT_STATE, F_QUESTION_TYPE)
STAGE1_CONVERSATION_FIELDS = STAGE1_FIELDS + (F_CONTEXT_AGE,)
STAGE1_ALL_FIELDS = STAGE1_CONVERSATION_FIELDS
...
STAGE1_CONVERSATION_SYSTEM_PROMPT = (
    STAGE1_SYSTEM_PROMPT
    + "\n"
    + "\n"
    "Вам дают всю переписку, а не одну реплику. Добавьте седьмой строкой:\n"
    "CONTEXT_AGE_MONTHS=<возраст из прошлых сообщений, если в последней реплике его нет, "
    "либо none>\n"
    "\n"
    "Правила для переписки:\n"
    "- Факты накапливаются. Возраст, названный три сообщения назад, остаётся в силе.\n"
    "- Новое перебивает старое. Если последняя реплика противоречит прошлой, верьте последней.\n"
    "- AGE_MONTHS - возраст из последней реплики. Если там его нет, ставьте none, "
    "а возраст из прошлых сообщений пишите в CONTEXT_AGE_MONTHS.\n"
    "- SYMPTOMS - только то, что беспокоит СЕЙЧАС. Симптом, который в переписке уже назван "
    "прошедшим или разрешившимся, сюда не попадает.\n"
    "- QUESTION_TYPE определяется по последней реплике, а не по всей переписке."
)

STAGE1_CONVERSATION_USER_TEMPLATE = (
    "Переписка, сверху старые сообщения:\n"
    "{conversation}\n"
    "\n"
    "Последняя реплика родителя:\n"
    "{case_text}"
)

HISTORY_ROLE_USER = "Родитель"
HISTORY_ROLE_ASSISTANT = "Ассистент"
HISTORY_ROLE_TITLES = {"user": HISTORY_ROLE_USER, "assistant": HISTORY_ROLE_ASSISTANT}
...
HISTORY_CONTEXT_BLOCK = (
    "Контекст этого разговора, собран из прошлых сообщений:\n"
    "{facts}\n"
    "Это уже известно: если возраст или показатели родитель называл раньше, не просите их заново. "
    "Решение принимайте по последнему сообщению. Тревожные признаки из прошлых сообщений сами по "
    "себе маршрут не поднимают."
)
...
STAGE2_SYSTEM_PROMPT = (
    ...
    "\n"
    "Факты могли быть собраны из всей переписки, а не из одной реплики. Тревожный признак из "
    "ПРОШЛЫХ сообщений сам по себе маршрут не поднимает: если ребёнок задыхался неделю назад, а "
    "сейчас вопрос про кашу, это не экстренный случай. Правило 1 применяется только к тому, что "
    "беспокоит сейчас. CONTEXT_AGE_MONTHS - возраст из прошлых сообщений, он остаётся в силе и "
    "используется наравне с AGE_MONTHS.\n"
    "\n"
