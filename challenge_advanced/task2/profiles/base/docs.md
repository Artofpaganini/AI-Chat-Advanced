...
`ux-writer-core` (**обязателен** для @TextWriter) · `ast-index` (читать реальный код) · `Context7`/DeepWiki (внешние API/версии — не по памяти) · `_shared/conventions.md` (base без project-context-скилла).
