...
| multi-dimension-review | **консилиум @Reviewer ×4** | sonnet | по измерению каждый (см. ниже), список кандидатов-findings с `file:line` |
| adversarial-verify | @Reviewer (иная линза) / мейн | opus | опровергнуть каждый finding ≥2 линзами; **сомнение → refuted** |
...
- **conventions** — конвенции проекта (см. `_shared/conventions.md` + `<context>-project-context`): **visibility = Critical**, нейминг моделей по слою, `_state.update`, запрет `!!`/`Any`/«DTO».
...
`ast-index` (читать контекст findings — callers/иерархия) · `superpowers:requesting-code-review` (чек-лист) · `<context>-project-context` + `_shared/conventions.md` · Sentry — если diff трогает known-crash зону · Context7/DeepWiki — при сомнении в API библиотеки.
