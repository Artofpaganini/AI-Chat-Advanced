...
- **build-variants:** релиз-варианты (напр. `assembleBetaDebug`/release, iOS archive, Docker image); проверка размера/минификации (R8) при необходимости.
...
| pre-flight | @Reviewer | по roster | ревью diff релиза + changelog против конвенций |
...
`ast-index` (что вошло в релиз) · `<context>-project-context` · `superpowers:verification-before-completion` (evidence before assertions) · `superpowers:finishing-a-development-branch` · по надобности `r8-analyzer` / `agp-9-upgrade` / `play-billing-library-version-upgrade`.
