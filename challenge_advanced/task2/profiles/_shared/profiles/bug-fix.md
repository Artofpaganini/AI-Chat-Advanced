...
|---|---|---|---|
| reproduce | @QA / @Dev + @DevOps (Bash) | Sonnet | воспроизвести по репорту/Sentry; зафиксировать шаги+окружение в `-reproduce.md` |
| diagnose | консилиум: diagnostics-линза · @Architect · security-линза · @DevOps | Opus | параллельно локализовать корень: стек, дифф, гонки, конфиг; синтез — оркестратор |
| fix | @Dev · @UIDev · @BackendDev | Sonnet | минимальный фикс корня (не симптома), по conventions |
| validation | @Reviewer + @DevOps (Bash) | Opus/Sonnet | зелёная сборка + regression: сценарий больше не падает, существующие тесты целы |
...
caveman. Skills: `<context>-project-context` · `<context>-udf-architecture` · `<context>-viewmodel` · `compose-principles`.
