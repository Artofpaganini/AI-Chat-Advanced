...
### Маркировка прогресса (обязательна при task execution)
Шаги в ответе визуально размечаются - иконка + **жирный** лейбл, каждая метка отдельной строкой, не сливать с прозой:

| Метка | Формат |
|---|---|
| Старт шага | `▶️ **ШАГ <n>/<N> · <стадия>** - <название шага>` |
| Задача шага | `🎯 **Задача:** <что именно делаем - одна строка>` |
| Итог шага | `✅ **Итог:** <главная мысль>` · `❌ **Провал:** <что сломалось + где>` · `⚠️ **Частично:** <что осталось>` |
| Переход стадии | `🔀 **Переход:** <стадия> -> <стадия>` · `⛓️ **Chaining:** <profile> -> <profile>` |

Между шагами - `---`. Без маркировки шаг считается незакрытым. Дублируется в каждом профиле (секция «Формат ответа»).

## Профили (режимы работы)

Тип задачи -> **профиль**, задающий весь флоу (стадии-DAG · сабагенты-по-стадиям · MCP/skills · MUST/MUST-NOT · формат ответа). Живут в `~/.claude/profiles/`. 16 универсальных: feature · bug-fix · refactor · migration · performance · research · review · test · security-audit · architecture · design · requirements · release · incident · docs · pr-check.

- Каталоги самодостаточны: `<context>/<name>.md` (затюненный флоу) + `<context>/roster.md` (стек+сабагенты: `base`/`xbet`/`alva`) + `<context>/conventions.md` (примеры кода). Один и тот же набор из 16 работает в любом контексте.
- **Выбор:** явно (`профиль: <name> · контекст: <base|xbet|alva> · размер: <XS|S|M|L|XL>`) или авто по ключевым словам запроса.
- **Chaining:** апстрим-профили запускают исполнительные - `design -> feature`, `requirements -> architecture|feature`, `architecture -> feature|refactor|migration`, `incident -> bug-fix`.
- Проектные профили/сабагенты/скиллы лежат в `<project>/.claude/` (git-excluded), симлинки в `~/.claude/`.
- Детали - `~/.claude/profiles/README.md` + секции «Оркестрация»/«Стадии задачи» этого файла.
...
- **Размерность (XS->XL) -> масштаб команды:** XS - session-only (1-2 сабагента); S/M - исполнитель -> ревьюер; L - Architect -> Dev(+UI) -> Reviewer; XL - full team + рекурсивные SubLead (глубина 2). Алгоритм: Understand -> Evaluate -> [Identify platforms] -> Decompose -> Assign -> Formulate -> Collect -> Verify.
- **Консилиум:** на стадиях research/diagnose - несколько сабагентов параллельно, каждый своей линзой (архитектура / безопасность / данные / UI / DevOps); оркестратор синтезирует, конфликт -> `AskUserQuestion`. Диверсити линз важнее избыточности.
- **Persistent-файлы:** состояние профиля/задачи - в `./swarm-report/<slug>-<profile>.md` (переживает compaction). Перед действием перечитать файл, сделанное отмечать `[x]`, пропускать выполненное.
- **Context-passing между стадиями:** в промпт сабагента явно - исходный запрос + саммари прошлой стадии + причина отката (если был). Чёткий I/O-контракт (что на входе / что на выходе).
- **Переходы стадий:** явно писать `Переход: <стадия> -> <стадия>`; разрешённые переходы - в профиле (DAG), прочие запрещены. Профили умеют чейнить друг друга (`Chaining: <A> -> <B>`).
...
## Текст, комментарии, KDoc, документация

Правила для любого текста, который я пишу: ответ в чате, док для пользователя, README, KDoc, комментарий.
Не зависит от того, пишу я документацию или просто отвечаю.
...
Дублируется в каждом профиле (секция «Формат ответа»). Правило «без комментариев и KDoc без явной просьбы»
остаётся в силе - эти правила говорят, **как** писать, если писать всё-таки просят.
