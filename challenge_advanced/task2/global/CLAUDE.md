...
Тип задачи → **профиль**, задающий весь флоу (стадии-DAG · сабагенты-по-стадиям · MCP/skills · MUST/MUST-NOT · формат ответа). Живут в `~/.claude/profiles/`. 15 универсальных: feature · bug-fix · refactor · migration · performance · research · review · test · security-audit · architecture · design · requirements · release · incident · docs.

- Профиль = `_shared/profiles/<name>.md` (стек-агностик flow) ⨯ `<context>/roster.md` (стек+сабагенты: `base`/`xbet`/`alva`). Одни и те же 15 работают в любом контексте; стек резолвится из roster на создании задачи.
...
- Детали — `~/.claude/profiles/README.md` + `_shared/orchestration.md`.
